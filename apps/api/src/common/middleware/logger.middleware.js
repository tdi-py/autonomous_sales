...
var LoggerMiddleware = function () {
    ...
    var LoggerMiddleware = _classThis = /** @class */ (function () {
        ...
        LoggerMiddleware_1.prototype.use = function (req, res, next) {
            var _this = this;
            ...
            res.on('finish', function () {
                ...
                var logFn = statusCode >= 500
                    ? _this.logger.error.bind(_this.logger)
                    : statusCode >= 400
                        ? _this.logger.warn.bind(_this.logger)
                        : _this.logger.log.bind(_this.logger);
                logFn("".concat(method, " ").concat(originalUrl, " \u2192 ").concat(statusCode, " (").concat(durationMs, "ms)"));
            });
            next();
        };
